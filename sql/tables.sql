...
CREATE FUNCTION dataset_is_dataset() RETURNS trigger as $$
       BEGIN
       IF EXISTS (SELECT id_type FROM objects WHERE id = NEW.dataset AND id_type = 'dataset') THEN
          RETURN NEW;
       ELSE
        RAISE EXCEPTION 'dataset is not of type dataset: %', (SELECT id_type FROM objects WHERE id = NEW.dataset)
       USING HINT = 'dataset uuids must refer to objects of type dataset';
       END IF;
       END;
$$ language plpgsql;

CREATE TRIGGER dataset_object_dataset_is_dataset BEFORE INSERT
       ON dataset_object FOR EACH ROW EXECUTE PROCEDURE dataset_is_dataset();
...
dataset uuid references objects(id) NOT NULL, -- FIXME we need a dataset independent version
...
addr_field text, -- can be null for addr_type == 'constant'
curator_note text,
...
CREATE TRIGGER obj_desc_inst_object_is_not_dataset BEFORE INSERT
       ON obj_desc_inst FOR EACH ROW EXECUTE PROCEDURE object_is_not_dataset();
