...
SELECT qv.value, NULL as open, NULL as controlled, u.label, a.label, id.label, im.formal_id, left_and_right_n(im.dataset::text, 4), left_and_right_n(qv.object_id::text, 4) FROM quant_values AS qv
...
SELECT NULL, cv.value_open, ct.label, cd.range::text, cd.label, id.label, im.formal_id, left_and_right_n(im.dataset::text, 4), left_and_right_n(cv.object_id::text, 4) FROM cat_values AS cv
