...
from quantdb.generic_ingest import back_populate_tables
...
def test_back_populate_simple_values_cat(session):
    ...
    values_inst = ValuesInst(type="subject", id_sub="sub-001", desc_inst=desc_inst.id, dataset=dataset_obj.id, id_formal="sub-001")
    values_inst = get_or_create(session, values_inst)

    # Create ValuesCat object without setting relationships (uses package)
    values_cat = ValuesCat(value_open="test-value", value_controlled=ct.id, object=package_obj.id, desc_inst=desc_inst.id, desc_cat=desc_cat.id, instance=values_inst.id)
...
def test_back_populate_simple_values_quant(session):
    ...
    desc_quant = DescriptorsQuant(label="test-quantitative", unit=unit.id, aspect=aspect.id, domain=desc_inst.id, shape="scalar", aggregation_type="instance")
    ...
    values_quant = ValuesQuant(
        value=42.5, value_blob={"value": 42.5, "unit": "test-unit"}, object=package_obj.id, desc_inst=desc_inst.id, desc_quant=desc_quant.id, instance=values_inst.id  # Required field
    )
