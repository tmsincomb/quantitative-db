...
def create_root_data(session):
    ...
    aspect_distance = Aspects(label="test-distance", iri="http://test.org/aspect/distance", description="Test distance aspect")
    ...
    desc_inst_human = DescriptorsInst(label="test-human", iri="http://test.org/class/human", description="Test human class")
    desc_inst_human = get_or_create(session, desc_inst_human)

    desc_inst_sample = DescriptorsInst(label="test-sample", iri="http://test.org/class/sample", description="Test sample class")
    desc_inst_sample = get_or_create(session, desc_inst_sample)

    # 6. Create Objects (root table)
    dataset_obj = Objects(id=str(uuid.uuid4()), id_type="dataset")
    dataset_obj = get_or_create(session, dataset_obj)

    package_obj = Objects(id=str(uuid.uuid4()), id_type="package", id_file=12345)
...
def create_intermediate_data(session, root_data):
    ...
    desc_cat = DescriptorsCat(domain=root_data["desc_inst_sample"].id, range="controlled", label="test-category", description="Test categorical descriptor")
    ...
    values_inst = ValuesInst(type="sample", desc_inst=root_data["desc_inst_sample"].id, dataset=root_data["dataset_obj"].id, id_formal="sam-test-001", id_sub="sub-test-001", id_sam="sam-test-001")
    values_inst = get_or_create(session, values_inst)

    # 4. Create ObjDescInst (depends on Addresses, DescriptorsInst, Objects)
    obj_desc_inst = ObjDescInst(object=root_data["package_obj"].id, desc_inst=root_data["desc_inst_sample"].id, addr_field=root_data["addr_tabular"].id, addr_desc_inst=root_data["addr_const"].id)
    obj_desc_inst = get_or_create(session, obj_desc_inst)

    # 5. Create ObjDescCat (depends on Addresses, DescriptorsCat, Objects)
    obj_desc_cat = ObjDescCat(object=root_data["package_obj"].id, desc_cat=desc_cat.id, addr_field=root_data["addr_tabular"].id)
    ...
    obj_desc_quant = ObjDescQuant(
        object=root_data["package_obj"].id, desc_quant=desc_quant.id, addr_field=root_data["addr_tabular"].id, addr_unit=root_data["addr_const"].id, addr_aspect=root_data["addr_const"].id
    )
    obj_desc_quant = get_or_create(session, obj_desc_quant)

    return {**root_data, "desc_cat": desc_cat, "desc_quant": desc_quant, "values_inst": values_inst, "obj_desc_inst": obj_desc_inst, "obj_desc_cat": obj_desc_cat, "obj_desc_quant": obj_desc_quant}
...
def test_back_populate_values_cat(session):
    ...
    # Don't set obj_desc_cat and obj_desc_inst - they have composite primary keys
    # and are handled through foreign key relationships
...
def test_back_populate_values_quant(session):
    ...
    values_quant = ValuesQuant(
        value=42.5, object=data["package_obj"].id, desc_inst=data["desc_inst_sample"].id, desc_quant=data["desc_quant"].id, instance=data["values_inst"].id, value_blob={"value": 42.5, "unit": "mm"}
    )
...
def test_back_populate_with_missing_parents(session):
    ...
    package_obj = Objects(id=str(uuid.uuid4()), id_type="package", id_file=99999)
    ...
    # Create ObjDescQuant
    obj_desc_quant = ObjDescQuant(object=package_obj.id, desc_quant=None, addr_field=None)  # Will be populated  # Will be populated
