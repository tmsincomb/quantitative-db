    ...
    def test_obj_desc_mappings_created(self):
        ...
        def patched_create_obj_desc_mappings(session, components, package_objects):
            ...
            for package in package_objects:
                ...
                obj_desc_inst = ObjDescInst(object=package.id, desc_inst=components["descriptors"]["nerve-volume"].id, addr_field=components["tabular_addr"].id)
    ...
    def test_back_populate_tables_used_for_leaf_tables(self):
        ...
        def patched_create_leaf_values(session, metadata, components, dataset_obj, package_objects, instances, mappings):
            ...
            for i, entry in enumerate(metadata["data"]):
                ...
                values_cat = ValuesCat(object=package_objects[i].id, term=components["terms"]["microct"].id, desc_inst=1, desc_cat=2)
                ...
                values_quant = ValuesQuant(object=package_objects[i].id, value=42.0, unit="mm³", desc_inst=1, desc_quant=3)
            ...
            with patch("ingestion.f006.ingest_objects_table", track_operation("ingest_objects")(lambda *a: (Mock(), []))):
                with patch("ingestion.f006.ingest_instances_table", track_operation("ingest_instances")(lambda *a: {})):
                    with patch(
                        ...
                        track_operation("create_mappings")(lambda *a: {"obj_desc_inst": [], "obj_desc_cat": [], "obj_desc_quant": []}),
                    ):
                        with patch(
