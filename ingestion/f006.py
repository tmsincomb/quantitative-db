...
def create_basic_descriptors(session):
    ...
    descriptors = [
        ...
        {"iri": "https://uri.interlex.org/tgbugs/uris/readable/quantdb/classes/nerve-cross-section", "label": "nerve-cross-section"},
    ]
    ...
    tabular_addr = session.query(Addresses).filter_by(addr_type="tabular-header", addr_field="volume", value_type="single").first()
    if not tabular_addr:
    ...
    modality_desc = DescriptorsCat(domain=created_descriptors["nerve-volume"].id, range="controlled", label="hasDataAboutItModality")
...
def create_obj_desc_mappings(session, components, package_objects):
    ...
    for package in package_objects:
        ...
        obj_desc_inst = ObjDescInst(object=package.id, desc_inst=components["descriptors"]["nerve-volume"].id, addr_field=components["const_addr"].id)
        ...
        obj_desc_cat = ObjDescCat(object=package.id, desc_cat=components["modality_desc"].id, addr_field=components["const_addr"].id)
        ...
        obj_desc_quant = ObjDescQuant(object=package.id, desc_quant=components["nerve_volume_desc"].id, addr_field=components["tabular_addr"].id)
...
def run_f006_ingestion(session=None, commit=False):
    ...
    try:
        ...
        # Create leaf table values using back_populate_tables
        leaf_values = create_leaf_values(session, metadata, components, dataset_obj, package_objects, instances, mappings)
        ...
        return {"dataset_obj": dataset_obj, "package_objects": package_objects, "instances": instances, "mappings": mappings, "leaf_values": leaf_values}
