...
class DescriptorsCat(Base):
    ...
    # Relationships
    domain_rel: Mapped[Optional['DescriptorsInst']] = relationship('DescriptorsInst', foreign_keys=[domain])
    values_cats: Mapped[List['ValuesCat']] = relationship('ValuesCat', uselist=True, viewonly=True)
    obj_desc_cats: Mapped[List['ObjDescCat']] = relationship('ObjDescCat', uselist=True, viewonly=True)
...
class DescriptorsQuant(Base):
    ...
    # Relationships
    aspect_rel: Mapped[Optional['Aspects']] = relationship('Aspects', foreign_keys=[aspect])
    domain_rel: Mapped[Optional['DescriptorsInst']] = relationship('DescriptorsInst', foreign_keys=[domain])
    unit_rel: Mapped[Optional['Units']] = relationship('Units', foreign_keys=[unit])
    values_quants: Mapped[List['ValuesQuant']] = relationship('ValuesQuant', uselist=True, viewonly=True)
    obj_desc_quants: Mapped[List['ObjDescQuant']] = relationship('ObjDescQuant', uselist=True, viewonly=True)
...
class Objects(Base):
    ...
    # Relationships
    id_internal_rel: Mapped[Optional['ObjectsInternal']] = relationship('ObjectsInternal', foreign_keys=[id_internal])
    values_quants: Mapped[List['ValuesQuant']] = relationship('ValuesQuant', uselist=True, viewonly=True)
    values_insts: Mapped[List['ValuesInst']] = relationship('ValuesInst', uselist=True, viewonly=True)
    values_cats: Mapped[List['ValuesCat']] = relationship('ValuesCat', uselist=True, viewonly=True)
...
class ObjectsInternal(Base):
    ...
    # Relationships
    dataset_rel: Mapped[Optional['Objects']] = relationship('Objects', foreign_keys=[dataset])
...
class ValuesCat(Base):
    ...
    # Relationships
    desc_cat_rel: Mapped[Optional['DescriptorsCat']] = relationship('DescriptorsCat', foreign_keys=[desc_cat])
    desc_inst_rel: Mapped[Optional['DescriptorsInst']] = relationship('DescriptorsInst', foreign_keys=[desc_inst])
    instance_rel: Mapped[Optional['ValuesInst']] = relationship('ValuesInst', foreign_keys=[instance])
    ...
    object_rel: Mapped[Optional['Objects']] = relationship('Objects', foreign_keys=[object])
    value_controlled_rel: Mapped[Optional['ControlledTerms']] = relationship('ControlledTerms', foreign_keys=[value_controlled])
...
class ValuesQuant(Base):
    ...
    # Relationships
    desc_inst_rel: Mapped[Optional['DescriptorsInst']] = relationship('DescriptorsInst', foreign_keys=[desc_inst])
    desc_quant_rel: Mapped[Optional['DescriptorsQuant']] = relationship('DescriptorsQuant', foreign_keys=[desc_quant])
    instance_rel: Mapped[Optional['ValuesInst']] = relationship('ValuesInst', foreign_keys=[instance])
    objdescinst: Mapped[Optional['ObjDescInst']] = relationship('ObjDescInst', foreign_keys=['object', 'desc_inst'])
