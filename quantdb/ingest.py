...
def check_parents_instances(instances, parents):
    ...
    parents_direct_ss = set((s, r[k]) for (d, s), r in instances.items() for k in ("id_sub", "id_sam") if k in r and s != r[k])
...
class getName:
    ...
    def valueCheck(self, value):
        if isinstance(value, dict):
            value = hash(frozenset((k, self.valueCheck(v) if isinstance(v, list) or isinstance(v, dict) else v) for k, v in value.items()))
...
def makeParamsValues(*value_sets, constants=tuple(), types=tuple(), row_types=tuple()):
    ...
    for i, values in enumerate(value_sets):
        ...
        values_template = ", ".join("(" + ", ".join(constants + tuple(":" + name for name in names)) + ")" for names, _ in proto_params)
...
class Queries:
    ...
    def desc_cat_from_label_domain_label(self, label, domain_label):
        ...
        res = [i for i, in self.session.execute(sql_text("select * from desc_cat_from_label_domain_label(:label, :domain_label)"), params)]
        if res:
            out = res[0]
            if out is None:
                raise ValueError(f"needed a result here {params}")
...
def ingest(dataset_uuid, extract_fun, session, commit=False, dev=False, values_args=None, **kwargs):
    ...
    if values_cv:
        for chunk in chunk_list(values_cv, batchsize):
            vt, params = makeParamsValues(chunk)
            session.execute(
                sql_text(f"INSERT INTO values_cat (value_open, value_controlled, object, desc_inst, desc_cat, instance) VALUES {vt}{ocdn}"),
                params,
            )
            if commit:
                session.commit()
...
def extract_reva_ft(dataset_uuid, source_local=False, visualize=False):
    if source_local:
        with open(
            pathlib.Path(f"~/.local/share/sparcur/export/datasets/{dataset_uuid}/LATEST/path-metadata.json").expanduser(),
            "rt",
        ) as f:
            blob = json.load(f)

        with open(
            pathlib.Path(f"~/.local/share/sparcur/export/datasets/{dataset_uuid}/LATEST/curation-export.json").expanduser(),
            "rt",
        ) as f:
    ...
    def make_vocd(this_dataset_updated_uuid, i):
        vocd = [
            ...
            (o, i.cd_obj, i.addr_const_null) for o, b in objects.items() if b["id_type"] == "package"  # XXX FIXME this is the only way I can think to do this right now ?
        ]
...
def values_objects_from_objects(objects):
    return [(i, o["id_type"], o["id_file"] if "id_file" in o else None) for i, o in objects.items() if o["id_type"] != "dataset"]  # already did it above
...
def ext_values(exts, ext_contents=None, dataset_metadata=None, process_record=None, tabular_header=True):
    ...
    if dataset_metadata:
        ...
        site_subject = {s["site_id"]: (s["specimen_id"] if s["specimen_id"].startswith("sub-") else sample_subject[s["specimen_id"]]) for s in dm["sites"]}
        for d, k, ik in asdf:
...
def extract_demo_jp2(dataset_uuid, source_local=False):
    ...
    def make_void(this_dataset_updated_uuid, i):
        ...
        void = [(o, i.id_nerve_cross_section, i.addr_const_null, None) for o, b in objects.items() if b["id_type"] == "package"]
        return void
...
def extract_demo(dataset_uuid, source_local=True):
    ...
    _dsp = "~/files/sparc-datasets/55c5b69c-a5b8-4881-a105-e4048af26fa5/SPARC/" "Quantified morphology of the human vagus nerve with anti-claudin-1/"
...
def extract_fasc_fib(dataset_uuid, source_local=True):
    ...
    (instances, _parents, objects, values_objects, values_dataset_object, values_q, values_c) = ext_values(exts, ext_contents, ir_dataset, process_record)

    parents_sam = [(dataset_id, s["sample_id"], p) for s in ir_dataset["samples"] for p in (s["was_derived_from"] if "was_derived_from" in s else (s["subject_id"],))]
