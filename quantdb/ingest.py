...
# Lazy import: identifier_indexes only needed for ingest_entity_metadata_all
...
_translate_species['NCBITaxon:9606'] = 'human'
...
def translate_species(v):
    if isinstance(v, dict):
        v = v['id']
    elif hasattr(v, 'curie'):  # Handle OntTerm objects
        v = v.curie
    elif hasattr(v, '__str__') and not isinstance(v, str):
        # Try to get string representation for OntTerm-like objects
        v = str(v)
        if v.startswith("OntTerm('"):
            # Extract curie from OntTerm('NCBITaxon:9606', ...)
            v = v.split("'")[1]

    return _translate_species.get(v, 'human')  # Default to human if not found
...
def ingest_entity_metadata_all(session, source_local=False, do_insert=True, batch=False, commit=False, dev=False):
    ...
    def get_ii():
        from sparcur.idmap import identifier_indexes  # Lazy import
